...
The LIVE INFERENCE toggle runs live CIR data through a trained seat classifier and drives the 3D occupancy from its predictions. `POST /api/inference/start` (`{model}`, chosen from `GET /api/inference/models` — the `.pt` checkpoints under `host-tools/seat-classification/models/`, newest first) spawns **one** persistent `live_infer_seats.py` process with the same `HEIMDALL_PYTHON`/`HEIMDALL_SEATCLASS_ROOT` configuration as the Training tab. The service assembles complete frames — 5 consecutive TDMA rounds containing all 20 directed links, `(20, 64)` magnitude matrices in the canonical link order of `build_seat_dataset.py` — skips incomplete frames, pipes them to the process as NDJSON, and broadcasts each prediction on the `seat-inference` WebSocket topic. Calibrated checkpoints require a frozen board; the service subtracts the frozen references exactly like the calibrated dataset variant.

In the browser, `LiveSeatFeed` (same `SeatFeed` interface as the mock) applies a majority vote over the last 5 confident predictions and a 0.6 confidence threshold — below it the UI shows RUNNING · UNCERTAIN and keeps the last stable seat. The model is a 4-class classifier with no "empty" class, so exactly one seat is occupied in live mode (noted in the panel). While live, the mock/manual test controls are disabled and marked overridden; STOP (`POST /api/inference/stop`) terminates the Python process (EOF on stdin, kill after a 2 s grace) and returns to mock mode. A crashed process or missing checkpoint surfaces as an error state, and closing every subscribed dashboard stops the run after a 30 s grace period.
