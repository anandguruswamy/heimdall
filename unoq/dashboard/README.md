...
Multilabel checkpoints (mode shown next to the model name and in the status readout) drive four independent per-seat detectors instead of one classifier: the backend smooths each seat bit over the same window and thresholds it (manifest `threshold`, default 0.5), the 3D scene fills every seat whose bit is on — including none for an empty cabin — and the confidence bars become four independent probabilities that do not sum to 100% (amber marks bits within ±10% of the threshold). Person identity is unavailable in this mode.

## Training tab

The Training tab captures labeled CIR clips and trains seat/person classifiers without leaving the dashboard. A classification-mode selector at the top switches between `Single person` (the default five-class flow, unchanged) and `Multi person (multi-label)`. Tags use the five seat classes above; in single mode occupied clips require a person label, while Empty clears and disables person. Tags persist in backend SQLite metadata; `seat: null` removes a tag and `exclude: true` leaves it out of training. Untagged and CIR-less clips never train.

In multi-person mode clips are tagged with seat checkboxes: any combination of the four seats (two or more store a `seats` array and disable person — identity is not recorded for multi-occupant clips) or the exclusive EMPTY chip. TRAIN exports the tagged clips using the `<Seat><Seat>TwoPeople` folder convention, runs `build_car_dataset_multilabel.py` over them, and trains with `--mode multilabel`; the metrics panel shows per-seat precision/recall/F1 plus subset (exact-match) and mean-bit accuracy instead of a confusion matrix. Selecting multi-seat clips while in single-person mode (or person labels containing a seat class name in multi mode) disables TRAIN with an explicit message, and the backend enforces the same rules.
