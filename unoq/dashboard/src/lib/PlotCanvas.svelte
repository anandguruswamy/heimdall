<script lang="ts">
  ...
  let { frame, label = 'Signal plot', animate = true, webgl = false }: { frame: () => PlotFrame; label?: string; animate?: boolean; webgl?: boolean } = $props();
  ...
  onMount(() => {
    ...
    const heatColor = (v: number): [number, number, number] => [Math.min(255, v * 420), Math.min(255, Math.max(0, v - 0.18) * 360), Math.min(255, (1 - v) * 115 + v * 50)];
    ...
    const drawGl = (f: PlotFrame) => {
      ...
      if (f.heatmap && f.heatWidth && f.heatHeight) {
        ...
        for (let i = 0; i < f.heatmap.length; i++) { const x=i%f.heatWidth,y=Math.floor(i/f.heatWidth),normalized=(f.heatmap[i]-(f.min??0))/Math.max(Number.EPSILON,(f.max??1)-(f.min??0)),c=heatColor(normalized); vertices.set([(x+.5)/f.heatWidth*2-1,(y+.5)/f.heatHeight*2-1,c[0]/255,c[1]/255,c[2]/255],i*5); }
...
<div class="plot"><canvas bind:this={canvas} aria-label={label}></canvas><span class="x-axis">{frame().xLabel ?? ''}</span><span class="y-axis">{frame().yLabel ?? ''}</span></div>
