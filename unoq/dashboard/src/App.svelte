<script lang="ts">
  ...
  const backendCaptureActive = $derived(clips.some((clip) => (clip.value as Record<string, unknown>)?.status === 'capturing'));
  ...
  async function saveClip() {
    if (clipState === 'capturing' || backendCaptureActive) return;
...
<div class="instrument">
  <header class="topbar">
    ...
    <div class="actions">
      ...
      <button class="icon-button" onclick={saveClip} disabled={clipState === 'capturing' || backendCaptureActive}>{clipState === 'capturing' ? `CLIP ${Math.round(clipProgress)}%` : backendCaptureActive ? 'CAPTURE ACTIVE' : `CAPTURE ${clipDurationS}S`}</button>
